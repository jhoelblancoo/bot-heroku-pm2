...
function YouTube() {
  ...
  this.search = function (
    ...
    return new Promise((resolve, reject) => {
      request(
        yt_api_auth(
          "search?maxResults=" + maxResults + "&part=" + part + "&q=" + query
        ),
  ...
  this.fetch_channel_uploads = function (channel_id, number_results = 5) {
    return new Promise((resolve, reject) => {
      request(
        yt_api_auth("channels?part=contentDetails&id=" + channel_id),
        function (error, response, body) {
          if (error) reject("ERROR: " + error);
            ...
            if (parsed_body.pageInfo.totalResults > 0) {
              var upload_id =
                JSON.parse(body).items[0].contentDetails.relatedPlaylists
                  .uploads;
              request(
                yt_api_auth(
